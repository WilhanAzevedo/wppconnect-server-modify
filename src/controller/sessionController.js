...
async function downloadFileFunction(message, client, logger) {
    try {
        ...
        let filename = '';
        if (message.type === 'ptt') {
            filename = path.resolve(__dirname, '../../', `WhatsAppImages/file${message.t}.oga`);
        } else {
            //result = `${filename}.${mime.extension(message.mimetype)}`;
            filename = path.resolve(
                __dirname,
                '../../',
                `WhatsAppImages/file${message.t}.${mime.extension(message.mimetype)}`
            );
        }
...
export async function deleteSessionFolder(req, res) {
    const { secretkey } = req.params;
    ...
    const path = `userDataDir/${session}`;
    if (fs.existsSync(path)) {
        fs.rmSync(path, { recursive: true, force: true });
        req.logger.info(`Session ${session} deleted`);
    }

    return res.status(200).json({ response: `Folder session ${session} deleted` });
}
...
export async function startSession(req, res) {
    ...
    if (fs.existsSync(path)) {
        fs.rmSync(path, { recursive: true, force: true });
        req.logger.info('Session already exists, removing...');
    }
...
export async function downloadMediaByMessage(req, res) {
    ...
    if (messageId.isMedia === true) {
        logger.info('Downloading media...1');
    ...
    } else if (messageId.type === 'ptt' || messageId.type === 'sticker' || messageId.type === 'document') {
        logger.info('Downloading media...2');
    ...
    logger.info('Downloading media...3');
    logger.info(result);
